...
int main(int argc, char * argv[]) {
  ...
  for ( unsigned int threads = threadUnit; threads <= maxThreads; threads += threadUnit ) {
    ...
    std::string * code = isa::Benchmarks::getFlopsOpenCL2x4(intIterations, typeName);
    
    try {
      kernel = isa::OpenCL::compile("flops2x4", *code, "-cl-mad-enable -Werror", *clContext, clDevices->at(clDevice));
